...
//		o.uv[7].xy = uv + right -up;
	...
	{	
		...
		float topL = Luminance( tex2D(_MainTex, i.uv[0] +i.uv[1]).rgb );
		float bottomL = Luminance( tex2D(_MainTex,i.uv[0] -i.uv[1]).rgb );
		float rightL = Luminance( tex2D(_MainTex, i.uv[0] +i.uv[2]).rgb );
		float leftL = Luminance( tex2D(_MainTex,i.uv[0] -i.uv[2]).rgb );
		float leftTopL = Luminance( tex2D(_MainTex, i.uv[0]+i.uv[1] -i.uv[2]).rgb );
		float leftBottomL = Luminance( tex2D(_MainTex, i.uv[0] -i.uv[1] -i.uv[2]).rgb );
		float rightBottomL = Luminance( tex2D(_MainTex, i.uv[0] +i.uv[1] +i.uv[2]).rgb );
		float rightTopL = Luminance( tex2D(_MainTex, i.uv[0] -i.uv[1] +i.uv[2]).rgb );
