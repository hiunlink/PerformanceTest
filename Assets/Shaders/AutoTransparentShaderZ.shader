Shader "Custom/AutoTransparentShaderZ" {
Properties {
    _Color ("Main Color", Color) = (1,1,1,1)
    _MainTex ("Base (RGB) Trans (A)", 2D) = "white" {}
    _OuterEdgeColor ("Edge Color", Color) = (0,0,0,1)
    _SelfLightFactor("Self Illumin Factor", float) = 0.8
    _RimPower ("Rim Power", float) = 3.0
    _EdgeWidth ("Edge Width", float) = 1.2
...
void surf (Input IN, inout SurfaceOutput o) {
	...
	half rim = _EdgeWidth - abs(dot (normalize(IN.viewDir), o.Normal));
	
	o.Emission = _SelfLightFactor*(_OuterEdgeColor.rgb * pow (rim, _RimPower));
